...
def read_example(rec_queue, pr, input_types):
  ...
  if pr.do_shift:
    ...
    idx = tf.random_uniform([1], 0, shape(choices, 0), dtype = tf.int64,seed=0)[0]
    ...
    shift_frame = start_frame_gt = tf.random_uniform(
      [1], 0, total_frames - num_slice_frames, dtype = tf.int32,seed=0)[0]
  ...
  if pr.augment_ims:
    ...
    r = tf.random_uniform(
      [2], 0, shape(ims, 1) - pr.crop_im_dim, dtype = tf.int32,seed=0)
    x, y = r[0], r[1]
    #x = tf.Print(x, [x, y])
  else:
    if hasattr(pr, 'resize_dims'):
  ...
  # moved
  if pr.augment_ims:
    ims = tf.cond(tf.cast(tf.random_uniform([1], 0, 2, dtype = tf.int64,seed=0)[0], tf.bool),
  ...
  def slice_samples(frame):
    ...
    if pr.augment_audio:
      ...
      delta = tf.random_uniform(
        [1], long(-dist*pr.samples_per_frame), long(dist*pr.samples_per_frame), dtype = tf.int64,seed=0)[0]
      start = start + delta
